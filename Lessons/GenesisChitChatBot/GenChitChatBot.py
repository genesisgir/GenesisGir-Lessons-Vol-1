...
the variable 'prompt' is storing '(press enter)' so that way we dont have to type it all the time or at least thats why
im using it. variables are created using assignmeant statemeants that contain the variable name the assignmeant operator
...
Pick the right variable name that has to do with the item or variable to begin with like the automatetheboringstuff if you
...
well designed by Genesis Gir this bot will hold conversations with you and remember all the facts you told it
so what are you waiting for it uses simple functions like input() to gather users inputs! The bot will ask you 
various questions that you will have to answer to gather data! Good luck on your journey with your questions
...
"""
                 ...
                 ::+::+:::--/--:+///++o+++oyyo+//:-:::-...:+:--::----:/:/+::....`....``.-:.``./+:/o+o//-----://:-`..:
                 ::+::o/:::-:--:+/+/+++sdhyydhhdddhs+::/-.-//-:----::-./+/-``.-:::-.`...-.```.:+/+++o+:-----///:-`.-/
                 :///:++:::::/-:o+++oyddo::hhhhdddhddhs:-.`.+:---:--.`:+:.```---:/osyyyyyyyso/++/+//s+:----:/+::...-:
                 ://+:+o/::::/--o/::oms:--shhyyyhhhhhhos.`.:::---.````..`````./shddhhhhhhhs/oyyss///o+----:/+//:`.-::
                 ///+//oo::::::-/::sms-...ysyoosssyysy:/:.---..``````````````.++hhhhhyhyyhh:--:oyo//o/---://+//:`-:::
                 ////+/oo/::::/:-/:om/..``s+oossssso-..`````````````````````...oyyssyyyyo--/---.yhh+o:--::/+///-.-::-
                 /////oo+o/::::/-/--+s.`  o+++/ssyo++h. ```````````````````````s+osyssss/--+...:hyo/o:-::/+/:+:.-::-.
                 /////+os+o/::::/-:-.-.   `o:-:++/--os  `````````````````````` o+/+ssss/::y/..-s/..:/--::+/:/+--:::--
                 //////+s++o/:::/+--....   `/+-.--:++````````````````````````` .s-:-/+:.-/s  .-.`.-/:-:://:/o/-::::-/
                 +:////++s+++o:::++:-....o/` `--:::```````````````````````````` -+....:-+o`  `..`.:+-::/+/:o+:://::++
                  -////+++o++s//::++/-.........```....`````````````````````````` `--:://- `:.....-/:::++/:+o////::oo
                   ://////+o/o:://:///:-..-.-.......``.``````````````````````````.------........-:/:/o///+o//////++
...
print(prompt)
input()
print('Your name '+name+' has '+str(len(name))+' characters in it by the way!')
print(prompt)# prompts user to press enter
input()# Takes input w/input()
# Hobby results
